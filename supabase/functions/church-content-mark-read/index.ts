...
type Payload = {
    content_id: string
    content_type: 'devotional' | 'message'
}
...
Deno.serve(async (req: Request) => {
    ...
    try {
        ...
        if (!payload?.content_id || !payload?.content_type) {
        ...
        if (error) {
            return jsonResponse({ ok: false, data: null, error: { message: 'Erro ao registrar leitura' }, meta: null }, 400, origin)
        }

...
Deno.serve(async (req: Request) => {
    ...
    try {
        ...
        if (messageError || !message) {
            return jsonResponse({ ok: false, data: null, error: { message: 'Mensagem nao encontrada' }, meta: null }, 404, origin)
        }

        const { data: series, error: seriesError } = await supabase
            .from('content_series')
            .select('id')
            .eq('id', message.series_id)
            .eq('church_id', profile.church_id)
            .maybeSingle()

        if (seriesError || !series) {
            return jsonResponse({ ok: false, data: null, error: { message: 'Mensagem nao pertence a igreja' }, meta: null }, 403, origin)
        }

        return jsonResponse({ ok: true, data: message, error: null, meta: null }, 200, origin)
    } catch (error: any) {

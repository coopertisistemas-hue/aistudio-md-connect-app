...
Deno.serve(async (req: Request) => {
    ...
    try {
        ...
        if (error) {
            return jsonResponse({ ok: true, data: { completed_days: [] }, error: null, meta: { empty: true } }, 200, origin)
        }

        return jsonResponse({ ok: true, data: data || { completed_days: [] }, error: null, meta: null }, 200, origin)
    } catch (error: any) {
